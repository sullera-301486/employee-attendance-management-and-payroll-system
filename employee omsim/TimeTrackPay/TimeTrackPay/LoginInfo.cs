...
namespace TimeTrackPay
{
    public partial class LoginInfo : Form
    {
        ...
        {
            if (dashboardEmployee == null)
            {
                dashboardEmployee = new DashboardEmployee();   //Create LoginPage
                dashboardEmployee.FormClosed += DashboardEmployee_FormClosed;  //Add eventhandler to cleanup
            }

            dashboardEmployee.Show(this);  //Show Form 
            Hide();
        }

        void DashboardEmployee_FormClosed(object sender, FormClosedEventArgs e)
        {
            dashboardEmployee = null;  //If form is closed make sure reference is set to null
            Hide();
        }
    }
}
